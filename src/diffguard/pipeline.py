    ...
    if language == Language.VUE:
        ...
        if tree is not None:
            scopes = _build_scopes(
                tree, diff_lines, language, source_lines, config.scope_size_limit, diff_file.is_new_file
            )
...
def _build_vue_context(
    ...
    """Build scopes and symbols for a Vue SFC via its extracted script block."""
    source_text = "\n".join(source_lines)
    script_result = extract_vue_script(source_text)
