...
- **Critical** (CVSS 9.0-10.0): Easily exploitable, severe impact \
— remote code execution, authentication bypass, full data breach.
- **High** (CVSS 7.0-8.9): Exploitable with significant impact \
— SQL injection, XSS with session hijack, privilege escalation.
- **Medium** (CVSS 4.0-6.9): Specific conditions required or moderate impact \
— CSRF, limited information disclosure, insecure defaults.
- **Low** (CVSS 0.1-3.9): Minor issues, limited impact \
— verbose error messages, minor information leaks.
- **Info** (CVSS 0.0): Best practice recommendations, no direct security impact.

## Confidence Levels
- **High**: Clear, well-established vulnerability pattern \
(e.g., unsanitized SQL concatenation, hardcoded credentials).
- **Medium**: Suspicious code but incomplete context \
(e.g., input validation may exist elsewhere).
